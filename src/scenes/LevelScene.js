...
export class LevelScene extends Phaser.Scene {
  ...
  create() {
    ...
    const bg = this.add.rectangle(W / 2, H / 2, W, H, Phaser.Display.Color.HexStringToColor(level.bgColor || '#3a3a44').color, 1);
    ...
    bus.once(EVENTS.LEVEL_COMPLETED, (payload) => {
      fadeScene(this, 'out', {
        onComplete: () => this.scene.start('ResultScene', {
          ...payload, levelIndex: this.levelIndex, levelName: level.name,
        }),
      });
    });
  }
