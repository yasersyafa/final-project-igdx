...
export class CutsceneScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.cards = (this.level && Array.isArray(this.level.cutscene)) ? this.level.cutscene : [];
  }
  ...
  create() {
    ...
    if (this.cards.length === 0) { this._finish(); return; }

    fadeScene(this, 'in');

    // Letterbox bars (top/bottom). Instant when reduced motion.
    const barTop = this.add.rectangle(0, 0, W, BAR_H, 0x000000, 1).setOrigin(0, 0).setDepth(10);
    const barBot = this.add.rectangle(0, H, W, BAR_H, 0x000000, 1).setOrigin(0, 1).setDepth(10);
    if (motionFlags.reduced) {
      barTop.y = 0; barBot.y = H;
    } else {
      barTop.y = -BAR_H; barBot.y = H + BAR_H;
      this.tweens.add({ targets: barTop, y: 0, ease: EASE.out, duration: DUR.base });
      this.tweens.add({ targets: barBot, y: H, ease: EASE.out, duration: DUR.base });
    }
    ...
    const zone = this.add.zone(0, 0, W, H).setOrigin(0, 0).setDepth(1)
  ...
  _showCard(i) {
    ...
    this.textTween = this.tweens.add({
      targets: this.card, alpha: 1, ease: EASE.out, duration: DUR.base,
      onComplete: () => {
  ...
  _next() {
    if (this.finished) return;
    if (this.holdEvent) { this.holdEvent.remove(false); this.holdEvent = null; }
    if (this.textTween && this.textTween.isPlaying()) { this.textTween.stop(); this.card.setAlpha(1); }

    if (this.index >= this.cards.length - 1) { this._finish(); return; }
    const nextIndex = this.index + 1;
    // Fade current card out, then show the next.
    this.tweens.add({
      targets: this.card, alpha: 0, ease: EASE.in, duration: DUR.quick,
  ...
  _finish() {
    ...
    if (this.holdEvent) { this.holdEvent.remove(false); this.holdEvent = null; }
    fadeScene(this, 'out', {
      onComplete: () => this.scene.start('LevelScene', { levelIndex: this.levelIndex }),
    });
  }
}
